...
export const saveTeamTool: ToolDefinition = {
  ...
  description: 'Create or modify a persistent team when the user asks. An existing team with the same name is overwritten. Pass exact available agent names for leader and members; unknown or unavailable names fail without saving the team. Display fields are optional. This tool never deletes teams.',
  inputSchema: {
    type: 'object',
    properties: {
      ...
      avatar: { type: 'string', description: 'Optional built-in icon reference, e.g. icon:chart-bar/blue, or a legacy emoji.' },
      requirePlanApproval: { type: 'boolean', description: 'Whether the leader waits for user approval before executing its plan. Defaults to false.' },
  ...
  execute: async (input, context) => {
    ...
    try {
      ...
      const fields = {
        ...
        description: text('description'), intro: text('intro'), avatar: text('avatar'), leaderNote: text('leaderNote'),
        expertise: lines('expertise'), samplePrompts: lines('samplePrompts'),

...
- **File writes in Abu's background process now do what they ask for.** The background process that runs tasks ignored a write's "only create a new file", "only if the file already exists", "append" and file-permission settings, so such a write could overwrite an existing file, recreate a deleted one, or replace a file it was meant to add to. No current feature relied on those settings there — saving experts and skills, which does, runs in the app window — so nothing was lost; the process now honors each of them, and refuses a write setting it cannot honor instead of skipping it.

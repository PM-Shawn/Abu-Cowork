...
- **An expert's tool list is now enforced the same way everywhere.** An expert whose file names no tools keeps inheriting the tools available at run time, and a list in the file still narrows it; that list is now checked when a tool is executed, not only in what the model sees, and a team leader's own list is no longer dropped when it takes the lead. Abu's own view of what is callable (the capability snapshot) and the runtime inventory now come from one place, and it reports connector tools by the names they are actually called by. Saving an expert with a tool name that does not exist is refused instead of silently kept.

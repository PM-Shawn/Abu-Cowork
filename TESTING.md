...
| Layer | Files |
|---|---|
| Unit (`*.test.ts(x)`, Vitest) | 677 |
| Integration (`*.integration.test.ts`) | 11 |
| Contract (`*.contract.test.ts`) | 11 |
| Enterprise (`enterprise-tests/**`, `npm run test:enterprise`, private build only — not in the default gate) | 36 |
...
| Node `node:test` scripts (`*.test.mjs` / `*.test.cjs`) | 64 |
<!-- test-inventory:end -->

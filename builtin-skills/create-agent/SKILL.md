...
- 工具不进草案的讨论范围：默认省略 `tools`，让代理继承运行时工具；不要把工具清单抄进角色说明或展示字段。
...
1. **起草推荐配置**：
   根据用户目标推荐这个代理负责什么、合适的记忆范围，连同名字、头像和上面的展示字段形成一份草案；已有明确授权就继续。
   工具按默认来：没有明确要求就省略 `tools`，让代理继承运行时工具；只有用户要求限制时才写清单，名字逐字取自 `capability_snapshot`。
...
下面是**用户明确要求限制工具**时的常见搭配；没有限制需求就省略 `tools`，让代理继承运行时工具。

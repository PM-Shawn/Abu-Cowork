...
- **agents/** — user-created agents; each has an AGENT.md file at ${agentPathTmpl}AGENT.md. Built-in agents and team members have NO file there. Do NOT construct a filesystem path from an agent name: the Available Agents list in this prompt is authoritative, and an agent that appears there needs no file read to be used.

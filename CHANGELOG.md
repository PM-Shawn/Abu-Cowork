...
- **Experts keep their team identity on every save path, and the team page tells missing members apart.** When Abu saves an existing expert for you from a conversation, the identity its team memberships point at is now carried over too; if the result would not read back with exactly that identity, nothing is written and the save is reported as failed. Whether an expert belongs to a plugin now comes from the installed-plugin records rather than a line in the expert's own file, so an edited or stale file can neither make a plugin's expert writable nor turn your own expert into a plugin one; a file left behind by an uninstalled plugin counts as yours. New experts get a creation time, so they are listed newest first as intended. On the team page, coming back to the Members tab no longer reopens a blank editor; each unavailable member now reads as its name ("reviewer" is unavailable) or as Unavailable member 1, 2… with the reason underneath, and can be removed even when no experts are left. The collapsed member strip in a team conversation also shows the unavailable count, and when every member is unavailable the leader gets one consistent instruction instead of two contradictory ones. Expert files saved with Windows line endings now have their last header field read correctly.
- **Detail views no longer show English labels in the Chinese UI.** The source and description labels in the expert, skill, and MCP detail views now follow the app language.

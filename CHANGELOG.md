...
- **Editing a project's expert or skill now changes it where it lives.** Editing an expert or skill that comes from a project folder, from the skills Abu writes for a project, or from the shared `~/.agents/skills` folder used to save a copy into Abu's own folder instead: your own expert or skill with the same name was silently overwritten, and the list kept showing the project's version, so the edit never appeared. Saving now writes into that item's own file, a rename moves its folder (scripts included) within the same place, and a rename that fails halfway puts the old text back so saving again works. Creating a new one also never replaces a file that appears under the same name at the same moment.

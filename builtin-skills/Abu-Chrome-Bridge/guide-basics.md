...
abu-browser-bridge__extract_text({ tabId: <number>, selector: ".article-body" })
```
- 不指定 selector：提取整页文本
- 指定 selector：只提取匹配元素的文本
- 适合文章、段落等纯文本内容

### 提取表格
```
abu-browser-bridge__extract_table({ tabId: <number> })
abu-browser-bridge__extract_table({ tabId: <number>, selector: "table.data" })
```
- 自动识别页面中的 HTML `<table>` 元素并返回表头与行数组
- 如果页面有多个表格，用 selector 指定目标表格
...
abu-browser-bridge__keyboard({ tabId: <number>, key: "a", modifiers: ["ctrl"] })
```
...
abu-browser-bridge__scroll({ tabId: <number>, selector: ".scroll-container", direction: "down" })
```
- direction: `up`、`down`、`left`、`right`
- 不指定 selector 时滚动整个页面

## 等待页面变化
```
abu-browser-bridge__wait_for({ tabId: <number>, condition: "{\"type\":\"appear\",\"locator\":{\"css\":\".result\"}}" })
abu-browser-bridge__wait_for({ tabId: <number>, condition: "{\"type\":\"textContains\",\"locator\":{\"css\":\"#status\"},\"text\":\"加载完成\"}" })
abu-browser-bridge__wait_for({ tabId: <number>, condition: "{\"type\":\"urlContains\",\"pattern\":\"/success\"}" })
```
- `type: "appear"` — 等待元素出现
- `type: "disappear"` — 等待元素消失（如 loading spinner）
- `type: "enabled"` — 等待元素可操作
- `type: "textContains"` — 等待元素包含指定文本
- `type: "urlContains"` — 等待 URL 包含 pattern
- 默认超时 30 秒

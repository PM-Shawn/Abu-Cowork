...
- **Editing an expert no longer silently drops it from its teams.** Saving an expert from the editor rewrote its file without the identity every team membership points at, so the next team run quietly left that member out while the team page still listed it. Edits and renames now keep that identity, and plugin-provided experts get one that is never written into the plugin's own files. A member that no longer resolves — deleted, or broken by an earlier edit — is now visible instead of silent: the team card counts only members that will actually be dispatched, the team detail and edit dialog mark it as unavailable with a Remove action, the leader is told its roster has a gap, and the member strip in a team conversation shows how many members are unavailable (click it to open the team panel). Deleting an expert that a team uses now asks first and names those teams, with a stronger warning when it leads one, since that team then stops working. Members already broken before this release are not repaired automatically; remove them from the team page.
- **Experts keep their team identity when Abu saves them, and the team page tells missing members apart.** When Abu saves an existing expert for you from a conversation, the identity its team memberships point at is now carried over too; if the result would not read back with exactly that identity, nothing is written and the save is reported as failed. Whether an expert belongs to a plugin now comes from the installed-plugin records rather than a line in the expert's own file, so an edited or stale file no longer decides whether an expert is treated as a plugin's; a file left behind by an uninstalled plugin counts as yours. Experts you create now get a creation time, so they are listed newest first as intended. On the team page, coming back to the Members tab no longer reopens a blank editor; each unavailable member now reads as its name ("reviewer" is unavailable) or as Unavailable member 1, 2… with the reason underneath, an unavailable leader is shown the same way, and members can be removed even when no experts are left. A team conversation opened right after launch no longer shows healthy members as unavailable while plugin records are still loading. The collapsed member strip also shows the unavailable count, a team card with one member says "1 member", and when every member is unavailable the leader gets one consistent instruction instead of two contradictory ones. Expert files saved with Windows line endings now have their last header field read correctly.
- **Saving an expert or skill can no longer overwrite or delete another one.** Creating an expert or skill under a name that is already taken — or renaming one onto such a name — used to replace the existing one's file, and a rename then deleted the renamed item's whole folder, including a skill's scripts and reference files and an expert's memory. The editors now refuse a name already in use (built-in and plugin names included, ignoring letter case) and say so under the name field; a rename moves the item's folder with everything in it; and a save that fails says so under the Save button instead of failing silently. Adding an expert to a team now records its team identity in the expert's own file, so a project-level expert is no longer copied out of, and deleted from, its project.
...
- **Long replies no longer make the conversation jump.** While a reply streams, the message you just sent stays put instead of being pushed around as the thinking and the answer grow — the residual movement v0.40.0 left open. The work-process header no longer pops in above "Thinking" half a second into a turn; a "Working for Ns" line now grows in smoothly, counts every second and becomes "Worked for Ns" when the turn ends. A finished reply also no longer leaves the view stuck a few pixels above the bottom.
- **A message sent as the previous reply wraps up is no longer dropped.** A message sent just as the previous reply was finishing — or a new task started from the welcome screen while the last one was still on its first reply — used to be discarded with only a brief notice, leaving the text in the input box; it is now queued and sent. When a sent message fails, you get the failed message with Retry, without a second copy also being put back in the input box.

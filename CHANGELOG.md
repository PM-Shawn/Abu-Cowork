...
- **An expert you create without a tool list can use the tools you have.** An expert whose file did not name any tools used to start with none, so a "researcher" without a `tools:` line could not search or read files. It now inherits the tools available at run time; a list in the file still narrows it. Abu's own view of what is callable (the capability snapshot) and the runtime inventory now come from one place, and it reports connector tools by the names they are actually called by. Saving an expert with a tool name that does not exist is refused instead of silently kept.

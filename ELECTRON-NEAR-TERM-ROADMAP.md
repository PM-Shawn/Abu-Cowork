...
Status: the original runtime roadmap and capability-model follow-up are merged
into `refactor-dev`. Consequential-action approval is implemented on its
isolated feature branch and remains unmerged pending attended acceptance and an
explicit merge decision. Windows real-machine package/runtime verification is
still required before release. This is not release approval.
...
2026-07-27 and explicitly merged branch 6 afterward. Branch 7 remains subject
to an explicit commit and merge decision.
...
`feat/electron-consequential-action-approval` is implemented and unmerged. It
has:

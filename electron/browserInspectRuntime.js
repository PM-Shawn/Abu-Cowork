// Abu Electron browser-tab element picker runtime.
//
// Electron main evaluates this file in a dedicated isolated world. The page
// remains sandboxed with no preload, Node integration, or IPC API. Main arms
// the picker and drains its bounded queue from that same world, so the page
// cannot observe or forge the per-session nonce.
//
// Vanilla ES5-ish JS, no build step or external dependencies. It still treats
// page-derived strings as untrusted and never assigns them via innerHTML.
(function () {
  'use strict';

  if (window.__ABU_BROWSER_INSPECT__) {
    // Re-injected into the same document — don't double-bind.
    return;
  }

  // Module state, set by Electron main through the isolated-world API below.
  ...
  // ---------- Return channel (isolated-world queue) ----------

  function sendSelection(payload) {
    if (!currentNonce) {
      return;
    }
    pendingSelections.push({ nonce: currentNonce, payload: payload });
    while (pendingSelections.length > 20) {
  ...
  window.__ABU_BROWSER_INSPECT__ = {
    setEnabled: function (enabled, nonce, labels) {
      currentNonce = enabled && typeof nonce === 'string' ? nonce : null;
      setEnabled(enabled, labels);
    },
    drainSelections: function () {
      var drained = pendingSelections;
      pendingSelections = [];
      return drained;
    }
  };

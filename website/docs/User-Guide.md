...
|------|------|
| **位置** | 设置 → 个人记忆 |
| **存储** | `~/.abu/agents/abu/memory.md` |
| **范围** | 跨所有项目生效 |
| **谁写** | 阿布在对话中自动积累，你也可以手动编辑 |
| **内容** | 你的名字、沟通偏好、常用工具等个人信息 |

**示例**：当你说「记住我叫 Shawn」，阿布会把这个信息存到个人记忆。下次新建对话时她仍然记得你是谁。
...
|------|------|
| **位置** | 右侧面板 → 项目记忆 |
| **存储** | `{工作区}/.abu/MEMORY.md` |
| **范围** | 仅当前工作区 |
| **谁写** | 阿布在对话中自动积累，你也可以手动编辑 |
| **内容** | 项目的技术栈、常见问题、架构特点等 |

**注意**：此文件由 AI 自动生成，**不建议提交到 git**。
...
| **范围** | 仅当前工作区 |
| **谁写** | 你手动编写 |
| **内容** | 编码规范、构建命令、团队约定等规则 |
| **优先级** | 最高 — 阿布会严格遵守 |

**建议提交到 git**，让团队成员共享同一套规则。

点击右面板的 **「项目指令 · 点击添加」** 即可编辑，支持 Markdown 格式。
...
对于大型项目，可以将规则拆分到 `{工作区}/.abu/rules/` 目录下的多个 `.md` 文件中，阿布会按字母顺序全部加载（最多 20 个文件）。
...
```
项目指令（最高）→ 项目记忆 → 个人记忆（最低）
```

如果项目指令和个人记忆有冲突，以项目指令为准。
...
| **搜索文件** | 按文件名模式搜索文件 |
| **高级查找** | 按内容、大小、时间等条件查找文件 |
| **文件监听** | 监听文件或目录的创建/修改/删除事件 |
...
| **发送通知** | 发送桌面通知提醒 |
| **剪贴板** | 读写系统剪贴板内容 |
| **电脑操控** | 截屏、点击、打字、滚动等桌面操作（需开启） |

### 内容生成

| 工具 | 功能 |
|------|------|
| **生成图片** | 根据文字描述生成图片（需配置图片生成服务） |
...
| **HTML 组件** | 创建交互式 HTML 小组件和嵌入式内容 |
| **主题工厂** | 10 套预设专业主题，可应用到任何文档/幻灯片/页面 |
| **品牌规范** | 应用品牌设计标准（配色、字体、视觉风格） |
| **Mermaid 图表** | 用 Mermaid 语法创建流程图、架构图、时序图等 |
| **信息图** | 创建信息图和数据可视化呈现 |
| **Slack GIF** | 为 Slack 创建优化的动画 GIF |
...
|------|------|
| **定时调度** | 创建和管理周期性定时任务 |
| **触发器** | 创建和管理事件驱动的自动化触发器 |
| **告警 SOP** | 标准操作流程和告警处理模板 |
...
让阿布直接操控你的电脑 — 截屏、点击、打字、滚动，就像有人在帮你操作一样。
...
- 每 5 分钟采样一次当前活跃窗口的 **应用名称**（如 Chrome、VS Code）
- 数据保留 7 天，超时自动清理
- 汇总为 3~5 行摘要注入对话上下文
...
不会。行为感知仅记录应用名称（如 Chrome、VS Code），不记录窗口标题或屏幕内容。原始数据存在本地，只有汇总摘要会注入对话上下文。可随时在设置中关闭并清除数据。

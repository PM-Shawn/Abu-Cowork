...
- **Scheduled tasks, triggers and IM runs can use the browser, and ask you over IM when they need to.** Turn on "Let scheduled tasks / triggers / IM use the browser" in Settings → Capabilities (it starts off), then set each kind of action — View pages, Click and fill in, Upload files, Run scripts — once to Allow, Ask every time or Deny: with you present "ask" is a dialog, while an automatic run sends an approval request over IM (a scheduled task or trigger sends it to its Results & approvals channel), refused if nobody answers within 5 minutes. Automatic runs act only on sites you set to always allow, never on pages recognized as payment, transfer or government services, stop after two refusals in a row, and leave a Browser task report in the conversation plus an outcome summary in the bound channel. Settings can also add a site by hand, preview what Abu may do at any address, list which automatic tasks can reach the browser, and show under each browser setting whether it actually saved.
...
- **Automatic tasks no longer use the browser until you switch them on.** In v0.42.0 a scheduled task, trigger or IM run could act on any site you had set to always allow. After updating, those runs are refused until you turn on the switch in Settings → Capabilities → Abu built-in browser → Automatic tasks, and the task report says the switch was off instead of claiming success.

...
- **Saving an expert or skill can no longer overwrite or delete another one.** Creating an expert or skill under a name that is already taken — or renaming one onto such a name — used to replace the existing one's file, and a rename then also deleted the renamed item's folder. The editors now refuse a name already in use (built-in and plugin names included, ignoring letter case) and say so under the name field. Renaming by letter case only (`Reviewer` → `reviewer`) no longer deletes the folder it has just saved into, and adding a project-level expert to a team or renaming one no longer deletes its folder inside the project.

...
Before the first `plugin_prepare` call, state the exact proposed package name to the user and explicitly explain that this name becomes permanently bound and cannot be changed afterward. Wait for the user to confirm that name before calling the tool. Do not infer confirmation from a general request to create a plugin. This confirmation is only needed before the first preparation; keep the bound name on subsequent edits.

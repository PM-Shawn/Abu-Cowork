...
- **Apps: Abu set up for one line of work, installed from the market or written in a conversation.** An app is a plugin whose manifest carries an `app` field: it gets an entry in the new app switcher at the top of the sidebar, a home page with modes, scenes and 3–6 ready-to-send templates per scene, a fixed expert team, expert or skill behind each scene, an app-level prompt that every conversation started inside it carries (captured at creation, so a later app update or removal never rewrites an existing conversation), its own sidebar navigation, and optionally its own web page shown in the main area (kept to the origins the app declares; other links open in the system browser; no access to your identity, conversations or files). Choose Discover apps in the switcher and the market shows only apps; Use opens the same review screen as a plugin install, now also listing the teams, navigation entries and web origins the app brings, and Install and enter takes you straight in. Three apps ship in the official market — Recruiting, Finance reconciliation and Content creation — each on one of the built-in expert teams. Plugins can now also ship expert teams (`teams/<id>.json`, referencing the package's own experts or Abu's built-in ones); they appear under Experts → Teams → Mine with the plugin named, read-only, and go away with the plugin. The Experts page opens on This app inside an app and can widen to All. A conversation started in an app keeps the app's mark in its title and in the conversation list, still opens after the app is uninstalled (with a removed notice), and the switcher remembers the app across restarts and lists the three most recently used. Create app — in the switcher or under Extensions → Plugins → Add — starts a creation conversation where Abu asks who the app is for, which scenes it offers and who handles each, writes the package (the `abu-plugin-builder` skill now carries the app and team formats and the catalog of built-in experts and teams), validates it with `plugin_prepare` (which now reports the teams and the app configuration), and Validate and preview installs it with Install and enter. Packages that use `app` or `teams/` declare `minAbuVersion`; an older Abu hides them in the market and marks an installed one as needing an upgrade. A marketplace entry marks an app with `providesApp: true`, and `npm run market:check` (also a CI step) validates the official market against the same rules the installer applies. The developer spec is in `docs/plugin-spec.md`; `examples/plugin-market/` holds a complete example app with a package team, a skill scene, a connector and a web page.

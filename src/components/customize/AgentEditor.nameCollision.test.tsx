 ...
 * wherever it lives, a project included — and only a rename moves the agent's
 * own folder, to its new name within the same parent (itemStorage.test.ts).
 * The last block runs the real storage layer to pin what an edit does for a
...
describe('AgentEditor — a new or renamed agent cannot take another agent\'s name', () => {
  ...
  it('allows renaming an agent to its own name in a different letter case, overwriting its own folder', async () => {
    ...
    expect(options).toEqual({ mustBeNew: false, renaming: true });
  });
  ...
  it('saves an agent whose unchanged name is, of course, its own', async () => {
    ...
    await waitFor(() => expect(vi.mocked(saveItemToAbuDir)).toHaveBeenCalledTimes(1));
    expect(vi.mocked(saveItemToAbuDir).mock.calls[0][5]).toEqual({ mustBeNew: false, renaming: false });
  });
...
describe('AgentEditor — editing an existing agent saves through its own folder (real storage layer)', () => {
  ...
  it('an ordinary save leaves a folder that is not named after the agent where it is', async () => {
  ...
  it('renaming the agent writes in place, then moves its folder (and so its memory.md)', async () => {
    onDisk(`${HOME}/.abu/agents/reviewer`, `${HOME}/.abu/agents/reviewer/AGENT.md`);
    const onSave = vi.fn(async () => undefined);
    render(<AgentEditor agent={reviewer} onClose={vi.fn()} onSave={onSave} />);
    ...
    await waitFor(() => expect(onSave).toHaveBeenCalledTimes(1));
    expect(writeTextFile).toHaveBeenCalledWith(`${HOME}/.abu/agents/reviewer/AGENT.md`, expect.stringContaining('name: renamed-agent'), { create: false });
    expect(rename).toHaveBeenCalledWith(`${HOME}/.abu/agents/reviewer`, `${HOME}/.abu/agents/renamed-agent`);
    expect(remove).not.toHaveBeenCalled();
  });

  it('when the rename cannot move the folder, the save fails visibly and the old text is put back', async () => {
    vi.spyOn(console, 'error').mockImplementation(() => {});
    // A letter-case-only rename: the must-be-new probe is skipped (on a
    // case-insensitive disk the manifest "at" the target is its own), so this
    // reaches the move — which the host refuses for an occupied folder.
    const cased: SubagentDefinition = { ...reviewer, name: 'Reviewer', filePath: `${HOME}/.abu/agents/Reviewer/AGENT.md` };
    // Only this agent answers to the name, in its own letter case.
    vi.mocked(agentRegistry.getAvailableAgents).mockReturnValue([{ name: 'Reviewer', description: '' }, known[1]]);
    onDisk(`${HOME}/.abu/agents/Reviewer`, `${HOME}/.abu/agents/reviewer`, `${HOME}/.abu/agents/reviewer/AGENT.md`);
    vi.mocked(rename).mockRejectedValueOnce(new Error('ENOTEMPTY: directory not empty'));
    const onSave = vi.fn(async () => undefined);
    render(<AgentEditor agent={cased} onClose={vi.fn()} onSave={onSave} />);
    fireEvent.change(nameInput(), { target: { value: 'reviewer' } });
    fireEvent.click(saveButton());
    ...
    expect(writtenPaths()).not.toContain(`${HOME}/.abu/agents/reviewer/AGENT.md`);
    expect(vi.mocked(writeTextFile).mock.calls.at(-1)).toEqual([`${HOME}/.abu/agents/Reviewer/AGENT.md`, 'original', { create: false }]);
    expect(remove).not.toHaveBeenCalled();
  });
});

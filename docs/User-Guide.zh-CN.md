...
> 批准计划只代表同意阿布开始按该方案工作，不等于提前允许最终的发送、发布、删除、购买等结果。电脑操控真正触发这些关键结果前，还会进行一次精确确认。
...
| 技能 | 标识 | 说明 |
|------|------|------|
| **Abu-Browser** | `Abu-Browser` | 操作阿布内置的可见浏览器，无需安装扩展；普通网页浏览、点击、填表和截图默认使用 |
| **Abu-Chrome-Bridge** | `Abu-Chrome-Bridge` | 通过 Chrome 扩展操作用户已有标签页、Cookie 和登录状态；仅在用户明确要求使用 Chrome 时启用 |
...
Abu 提供两种用户可见的浏览器能力，二者不会静默切换：

| 方案 | 适用场景 | 启动方式 |
|------|---------|---------|
| **阿布内置浏览器**（默认） | 普通网页浏览、点击、填表、截图和内容提取 | Electron 客户端内置，无需安装扩展 |
| **我的 Chrome** | 明确需要已有 Chrome 标签页、Cookie、扩展或登录状态 | 在 **设置 → 能力 → 我的 Chrome** 中点击「连接 Chrome」 |
| **Webapp Testing** | 本地 Web 应用的端到端测试，需要独立测试环境 | 安装 `webapp-testing` 技能，使用 Playwright |
...
阿布会在应用工作区中打开可见浏览器标签。该浏览器使用独立会话，不会读取你的 Chrome Cookie 或登录状态。

### 使用我的 Chrome

当你明确说“用我现在的 Chrome”“操作已有标签页”或“复用登录状态”时，阿布才会选择 `Abu-Chrome-Bridge`。如果扩展未连接，任务会停下并提示连接步骤，不会自动改用内置浏览器。

### 连接 Chrome

1. 打开 **设置 → 能力 → 我的 Chrome**，点击「连接 Chrome」
2. 点击「打开安装窗口」，Abu 会自动准备本机连接，并同时打开 Chrome 扩展管理页和随应用提供的扩展文件夹
3. 在 Chrome 中开启「开发者模式」，点击「加载已解压的扩展程序」，选择已打开的 `browser-extension` 文件夹
4. 返回 Abu；连接状态会自动复检，也可以点击「检查连接」

正式版也使用随 Abu 提供的本地扩展，不通过 Chrome 应用商店分发。Chrome 出于安全要求，仍需你亲自完成「开发者模式」「加载已解压的扩展程序」和选择文件夹；其余准备和连接检查由 Abu 完成。扩展需要读取和操作所有网站中的页面，并管理下载，以完成你明确交给 Abu 的浏览器任务。请只在可信设备上安装；不用时可在 Abu 中点击「断开我的 Chrome」，并在 Chrome 扩展管理页停用或移除扩展。

普通用户不需要理解或手动配置 Skill、MCP 和启动命令。当任务需要使用现有 Chrome 而这项能力处于断开状态时，Abu 会暂停当前这一步浏览器操作并直接打开同一连接引导，但不会自行重新连接。你需要主动点击「连接 Chrome」并完成设置；扩展连接成功后，Abu 会在原任务中自动恢复刚才暂停的操作，不用重新描述需求，也不用再发送「继续」。
...
1. 打开 **设置** → **能力**
2. 找到 **「电脑操控」**，点击「开启」
3. 按引导分别为「查看屏幕」和「操作界面」点击「打开系统设置」
4. 授权后返回 Abu；页面会自动复检，只有两项都允许时才会显示为已就绪
...
> Windows 截图通常不需要额外权限；控制高权限窗口时需要以管理员身份运行 Abu。
...
Abu 的电脑操控不是直接裸调用输入能力，而是由 Electron 主进程和原生辅助组件执行多层守护：

#### 1. 应用授权与敏感应用拦截

电脑操控授权只对当前任务和当前应用有效：

- **系统红线**：钥匙串访问、系统设置、活动监视器、终端、iTerm 2、Warp、VS Code 等直接拒绝，任何权限模式都不能绕过
- **网页和通讯应用**：浏览器、微信、消息、Mail、Outlook、Slack、飞书、钉钉、Discord、Zoom 等，每个任务都需要用户明确授权
- **普通应用**：Finder、备忘录、计算器和 Office 等根据当前权限模式决定是否首次询问

任务停止、结束、Abu 重启或目标应用变化后，授权自动失效；V1 不提供永久“始终允许”。

#### 2. 关键操作二次确认

应用授权只允许阿布查看或操控该应用，不代表允许其中的所有结果。即使已经批准执行计划，或当前选择“替我审批 / 完全自主”，以下操作真正发生前仍会弹出原生确认窗口：
...
弹窗会显示目标应用和具体结果，默认焦点在“取消”。“允许这一次”只对紧接着的一次原生输入有效，不能被下一次操作复用。阿布还会独立检查辅助功能控件的名称；即使操作声明成普通点击，命中“发送”“删除”“支付”等已知按钮也会要求确认。
...
普通网页任务直接描述即可，Abu 会使用无需扩展的内置浏览器。只有需要现有 Chrome 标签页、Cookie、扩展或登录状态时，才在 **设置 → 能力 → 我的 Chrome** 中点击「连接 Chrome」并按页面完成实验性扩展安装。
...
在 **设置 → 能力 → 电脑操控** 中点击「继续设置」，分别检查「查看屏幕」和「操作界面」。macOS 会直接打开对应的 **系统设置 → 隐私与安全性** 页面，返回 Abu 后会自动复检；如果授权后仍显示未允许，再重启 Abu。

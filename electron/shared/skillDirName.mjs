 ...
 * It lives here, next to the other shared package rules, so the Node-side
 * package scan (`pluginPackageScan.mjs`, used by the marketplace check) reads
 * a package by the same rule the renderer installs it by; `src/core/skill/
 * skillDirName.ts` re-exports it for the renderer.
 */
 ...
 *
 * The character sets are written as code points rather than as escapes inside a
 * regular expression so that the rule reads the same in every editor and no
 * control character ever sits in this file as a byte.
 */
function isRefusedCodePoint(code) {
    // C0 and NUL, DEL, and C1.
    if (code < 0x20 || (code >= 0x7f && code <= 0x9f)) return true;
    // ZWSP, LRM, RLM.
    if (code === 0x200b || code === 0x200e || code === 0x200f) return true;
    // The bidi embeddings and overrides, then the isolates.
...
export function isSafeSkillDirName(name) {
    ...
    for (const character of name) {
        if (isRefusedCodePoint(character.codePointAt(0))) return false;
    }
    return true;
}

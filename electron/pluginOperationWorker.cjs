 ...
 * Same file?
 *
 * Takes a live `Stats` on either side, or the frozen `{ ino, dev }` the parent
 * sent over JSON. Every stat below is taken with `bigint: true` so the ids are
 * exact: a Windows file id is 64 bits wide, and `Stats.ino` rounds it into a
 * double where two files one MFT record apart become the same number
 * (`electron/fileIdentity.cjs`).
 */
const same = (a, b) => sameIdentity(identityOf(a), identityOf(b));

/** All mutations use single-component paths relative to an inode-pinned cwd. */
function run(input, io = fs, chdir = process.chdir) {
  if (!same(io.statSync('.', { bigint: true }), input.identity)) throw new Error('Plugin operation: profile changed');
  ...
  const parent = io.statSync('.', { bigint: true });
  ...
  if (input.action === 'ensure') {
    ...
    const reset = () => { chdir(base); if (!same(parent, io.statSync('.', { bigint: true }))) throw new Error('Plugin operation: parent changed'); };
    ...
    const identities = new Map([['', io.lstatSync(input.temp, { bigint: true })]]);
    for (const [parts, bytes] of entries) {
      reset(); enterDirectory(input.temp, false, io, chdir);
      if (!same(identities.get(''), io.statSync('.', { bigint: true }))) throw new Error('Plugin operation: staging changed');
      for (let i = 0; i < parts.length - 1; i++) {
        enterDirectory(parts[i], false, io, chdir);
        if (!same(identities.get(parts.slice(0, i + 1).join('/')), io.statSync('.', { bigint: true }))) throw new Error('Plugin operation: tree directory changed');
      }
      const name = parts.at(-1);
      if (bytes === null) { io.mkdirSync(name); identities.set(parts.join('/'), io.lstatSync(name, { bigint: true })); }
      else {
    ...
    reset();
    const staged = io.lstatSync(input.temp, { bigint: true });
    ...
    const before = io.lstatSync(input.from, { bigint: true });
    ...
    if (input.action === 'archive') {
      ...
      if (!before.isFile() || before.nlink > 1 || size > 8 * 1024 * 1024) throw new Error('Plugin operation: invalid archive source');
      ...
      try {
        ...
        const current = io.lstatSync(input.from, { bigint: true });
        if (offset !== size || !same(current, before) || current.isSymbolicLink()
  ...
  if (!same(parent, io.statSync('.', { bigint: true }))) throw new Error('Plugin operation: directory changed');

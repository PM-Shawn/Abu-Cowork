...
function isValidNewAvatar(value: string): boolean {
  if (!value) return true;
  const parsed = parseAvatarValue(value);
  if (parsed.kind === 'icon') return true;
  if (parsed.kind !== 'emoji' || value.length > 64) return false;
  // Validate new writes without rewriting stored legacy values. Count visible
  // graphemes, not code points: skin tones, flags and ZWJ families stay intact.

...
| Layer | Files |
|---|---|
| Unit (`*.test.ts(x)`, Vitest) | 736 |
| Integration (`*.integration.test.ts`) | 12 |
| Contract (`*.contract.test.ts`) | 25 |
| Enterprise (`enterprise-tests/**`, `npm run test:enterprise`, private build only — not in the default gate) | 38 |
| Quarantined (`src/__tests__/quarantine/`) | 0 |
| Quarantined specs (`e2e/**/*.spec.ts`, `tests/e2e/**/*.spec.ts`) | 0 |
| Web E2E (`e2e/*.spec.ts`, Playwright) | 8 |
| Real-Electron E2E (`tests/e2e/*.spec.ts`) | 49 |
| Node `node:test` scripts (`*.test.mjs` / `*.test.cjs`) | 78 |
<!-- test-inventory:end -->

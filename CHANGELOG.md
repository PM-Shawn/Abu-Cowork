...
- **Pick an expert's or team's avatar from the built-in icon set.** The editor's avatar field is now a picker: 20 line icons in 6 tints, or leave it on the default. An avatar written by Abu itself (`icon:<icon>/<tint>`) renders as that icon instead of as raw text, and an emoji you typed earlier still shows. A value that is neither is refused when an expert is saved. The built-in and marketplace experts now come with their own icons instead of the generic robot mark. The avatar now follows the expert or team everywhere it appears: it fills the card and the details header rather than sitting inside a small grey square, and it shows on the chip in the message box and in the list that drops down when you type @.
